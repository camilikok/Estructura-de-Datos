...
public class AStar
{
    ...
    public Cell fixTarget(Cell initialTarget) {
        ...
        HashSet<Cell> closedSet = new HashSet<>(); //visitados
        while(!openSet.isEmpty()){ //validar contenido  cola si no esta vacia
            Cell currentCell = openSet.poll(); //extrae la celula actual
            closedSet.add(currentCell); //marcamos como visitada
            if (currentCell.equals(initialTarget))
                return currentCell; //si es igual al objetivo se devuelve el valor, si no se sigue con las venicinitas

            ArrayList<Cell> neighbors = game.getNeighbors(currentCell);
            for (Cell n : neighbors) {
                if (closedSet.contains(n)) continue; //valida si ya se visito
                if (game.getGameSpace(n.getX(), n.getY()) == 0) continue; // si es igual a un obstaculo 0 se omita
                openSet.add(n); //si es valida se agrega al opense
            }
        }
        return null;
    }
